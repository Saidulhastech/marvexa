...
**Hybrid content collections (`compare`, `lookbook`).** A few collections are *editorial layout + live Shopify data*: the YAML holds only what Shopify can't (coordinates, labels, curation order), and products resolve from the Storefront API at request time. The "Shop the Look" section ([`src/components/home/marvexa/Lookbook.astro`](src/components/home/marvexa/Lookbook.astro)) is the reference: each YAML look entry has a photo, label and a few hotspots; every hotspot `handle` pins a real product (omit it → falls back to a best-seller, no repeats). Resolution happens in [`src/pages/index.astro`](src/pages/index.astro) (fetch-in-page, pass-as-prop — same as the other dynamic home sections) and the component maps each product to **inline size/colour variant pickers**. Picking a size resolves the row's variant id client-side; per-item "+ Bag" and "Add Complete Look" both add real variants through the nanostore cart (`addItem`), adding sequentially so the store's `seq` guard commits every line. Creating a Shopify product does **not** auto-add it to a look — curation (which product, which hotspot) stays in YAML by design.
